...
check_governance() {
  ...
  local missing=0
  for f in "$ROOT/AGENTS.md" "$ROOT/skills/README.md" "$ROOT/policies/README.md" "$ROOT/agents/README.md" "$ROOT/agents/website/AGENT.md" "$ROOT/agents/website/AGENT.json" "$ROOT/scripts/query.sh" "$ROOT/scripts/playbook.sh" "$ROOT/scripts/work.sh" "$ROOT/scripts/webterm-ui.sh" "$ROOT/ops/webterm-ui/server.py" "$ROOT/docs/work/README.md" "$ROOT/docs/ops/code-haloeddepth-com.md" "$ROOT/scripts/hooks.sh" "$ROOT/.githooks/pre-push"; do
    if [ ! -s "$f" ]; then
      echo "Missing or empty: $f"
      missing=1
    fi
