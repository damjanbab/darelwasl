...
(defn create-case!
  ...
  (or
   ...
   (let [db0 (d/db conn)
         ...
         raw-service (param-value params :service/id)
         service-id (cond
                      (keyword? raw-service) raw-service
                      (string? raw-service) (let [s (str/trim raw-service)]
                                              (cond
                                                (str/blank? s) nil
                                                (str/starts-with? s ":") (keyword (subs s 1))
                                                :else (keyword s)))
                      :else nil)
         {title :value title-err :error}
         (normalize-string (param-value params :service.case/title)
                           "title"
                           {:required false :allow-blank? false})
         {assignee-id :value assignee-err :error}
         (normalize-uuid (param-value params :service.case/assignee) "assignee id")
     ...
     (cond
       ...
       (nil? service-id) (error 400 "service/id is required")
       ...
       :else
       (let [{:keys [error]} (client-eid db0 client-id ws)]
         (if error
           error
           (let [case-id (UUID/randomUUID)
                 ...
                 case-tx (-> {:service.case/id case-id
                             ...
                             (cond-> assignee-id (assoc :service.case/assignee [:user/id assignee-id]))
                             (entity/with-ref db0))
                 ...
                 step-txs (mapv (fn [{:keys [id order owner public internal]}]
                                  (let [public-phase (:phase public)
                                        ...
                                        tx (-> {:service.case.step/id (UUID/randomUUID)
                                               ...
                                               (entity/with-ref db0))]
                                    (cond-> tx
                                      (nil? public-next-action) (dissoc :service.case.step/public-next-action)
                                      (nil? public-label) (assoc :service.case.step/public-label (name id)))))
                                steps)
             ...
             (if (:error tx-res)
               ...
               (let [db1 (d/db conn)]
                 {:case (present-case (fetch-case* db1 case-id ws))
