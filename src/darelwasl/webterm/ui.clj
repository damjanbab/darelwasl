    ...
    (str "<!doctype html><html><head><meta charset=\"utf-8\" />"
         ...
         "</style></head><body>"
	         "<div class=\"bar\">"
		         ...
		         "<button class=\"btn\" id=\"b-review\" type=\"button\">Proof</button>"
		         "<button class=\"btn\" id=\"b-signoff\" type=\"button\">Sign off</button>"
		         ...
		         "<input id=\"refbox\" type=\"text\" placeholder=\"ref…\" style=\"width:240px\" />"
		         "<button class=\"btn\" id=\"b-ref-copy\" type=\"button\">Copy</button>"
		         "<button class=\"btn\" id=\"b-ref-paste\" type=\"button\">Paste→term</button>"
	         "<span class=\"sp\"></span>"
         ...
         "const CANARY=" lab-canary-session ";"
	         ...
	         "function apiSess(path,sess){const sep=path.includes('?')?'&':'?';return UI_PREFIX+path+sep+'session='+encodeURIComponent(String(sess))}"
         ...
         "function isText(e){return ['.txt','.md','.markdown','.log','.json','.edn','.csv'].includes(e)}"
	         ...
	         "function setRef(t){refText=String(t||''); const rb=document.getElementById('refbox'); if(rb) rb.value=refText;}"
	         ...
	         "async function pasteToTerm(t){const text=String(t||''); if(!text) return false; try{await fetch(api('/api/lab/tmux/paste'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:text})}); return true;}catch(e){return false;}}"
	         ...
	         "else if(e==='.html'||e==='.htm'){inner='<div class=\"paper\"><iframe sandbox=\"allow-forms allow-popups allow-popups-to-escape-sandbox\" src=\"'+url+'\" title=\"html\"></iframe></div>';} "
	         ...
	         "function outboxViewUrl(sess,name){return apiSess('/api/lab/outbox/view?name='+encodeURIComponent(String(name||'')), sess)}"
	         "function outboxDownloadUrl(sess,name){return apiSess('/api/lab/outbox/download?name='+encodeURIComponent(String(name||'')), sess)}"
	         "async function viewOutbox(sess,name,label){const ref='outbox:'+String(sess)+'/'+String(name||''); const e=extLower(name); const url=outboxViewUrl(sess,name); const dl=document.getElementById('p-dl'); dl.href=outboxDownloadUrl(sess,name); dl.style.display='inline-flex';"
	         "let inner=''; if(e==='.pdf'){inner='<div class=\"paper\"><iframe src=\"'+url+'\" title=\"pdf\"></iframe></div>';} "
	         "else if(e==='.html'||e==='.htm'){inner='<div class=\"paper\"><iframe sandbox=\"allow-forms allow-popups allow-popups-to-escape-sandbox\" src=\"'+url+'\" title=\"html\"></iframe></div>';} "
	         "else if(isImg(e)){inner='<div class=\"paper\"><img src=\"'+url+'\" style=\"width:100%;height:auto;display:block\" /></div>';} "
	         "else if(isText(e)){inner='<div class=\"paper\"><pre id=\"tpre\">(loading…)</pre></div>';}"
	         "else{inner='<div class=\"sub\">Not previewable. Use Download.</div>';}"
	         "openPanel((label||'Outbox')+' · '+ref, inner); if(isText(e)){try{const r=await fetch(url,{cache:'no-store'}); document.getElementById('tpre').textContent=await r.text();}catch(err){document.getElementById('tpre').textContent='Failed: '+err;}} }"
         ...
         "if(!items.length){ul.innerHTML='<div class=\"sub\">No work items found.</div>'; return;}"
         ...
         "const bOpen=document.createElement('button'); bOpen.className='btn'; bOpen.type='button'; bOpen.textContent='Open'; bOpen.addEventListener('click', ()=>openWork(it.id));"
	         "const bCopy=document.createElement('button'); bCopy.className='btn'; bCopy.type='button'; bCopy.textContent='Copy PR ready'; bCopy.addEventListener('click', async ()=>{const t='PR ready '+it.id; setRef(t); await copyText(t);});"
         ...
         "document.getElementById('wpre').textContent=txt; document.getElementById('wsel').addEventListener('click', ()=>{setWork(wid); closePanel();});"
	         "document.getElementById('wref').addEventListener('click', async ()=>{const t='workfile:'+wid; setRef(t); await copyText(t);});}"
         ...
         "const bV=document.createElement('button'); bV.className='btn'; bV.type='button'; bV.textContent='View'; bV.addEventListener('click', ()=>viewRef(it.ref,'File'));"
	         "const bC=document.createElement('button'); bC.className='btn'; bC.type='button'; bC.textContent='Copy ref'; bC.addEventListener('click', async ()=>{setRef(it.ref); await copyText(it.ref);});"
	         "right.appendChild(bV); right.appendChild(bC); li.appendChild(left); li.appendChild(right); li.addEventListener('click', ()=>{setRef(it.ref); viewRef(it.ref,'File')}); ul.appendChild(li);} }"

	         "async function showReview(){"
	         "const wid=String(workId||'').trim();"
	         "if(!wid){openPanel('Proof', '<div class=\"sub\">Pick a work first (Work → Select).</div>'); return;}"
	         "openPanel('Proof · '+wid,"
	         "'<div class=\"sub\">Proof artifacts are delivered to the Lab outbox as <code>work-proof-'+wid+'.html</code> and <code>work-links-'+wid+'.txt</code>.</div>' +"
	         "'<div style=\"height:10px\"></div>' +"
	         "'<div class=\"row\">' +"
	         "'<input id=\"pf-w\" type=\"text\" placeholder=\"work id\" style=\"width:260px\" />' +"
	         "'<button class=\"btn\" id=\"pf-set\" type=\"button\">Set</button>' +"
	         "'<button class=\"btn\" id=\"pf-refresh\" type=\"button\">Refresh</button>' +"
	         "'<span class=\"sub\">status:</span><span class=\"sub\" id=\"pf-status\">(loading…)</span>' +"
	         "'</div>' +"
	         "'<div style=\"height:10px\"></div>' +"
	         "'<div class=\"row\">' +"
	         "'<button class=\"btn\" id=\"pf-open-proof\" type=\"button\">View proof</button>' +"
	         "'<button class=\"btn\" id=\"pf-open-links\" type=\"button\">View links</button>' +"
	         "'<button class=\"btn\" id=\"pf-open-failure\" type=\"button\">View failure</button>' +"
	         "'<button class=\"btn primary\" id=\"pf-approve\" type=\"button\">Approve proof → PR</button>' +"
	         "'<button class=\"btn\" id=\"pf-copy-pr\" type=\"button\">Copy PR URL</button>' +"
	         "'<a class=\"btn\" id=\"pf-open-pr\" href=\"#\" target=\"_blank\" rel=\"noreferrer\" style=\"display:none\">Open PR</a>' +"
	         "'<a class=\"btn\" id=\"pf-required\" href=\"#\" target=\"_blank\" rel=\"noreferrer\" style=\"display:none\">Open required</a>' +"
	         "'<span class=\"sub\" id=\"pf-st\"></span>' +"
	         "'</div>' +"
	         "'<div style=\"height:10px\"></div>' +"
	         "'<div class=\"sub\">Outbox:</div><div style=\"height:6px\"></div><ul class=\"list\" id=\"pf-list\">(loading…)</ul>'"
	         ");"
	         "document.getElementById('pf-w').value=wid;"
	         "let prUrl=''; let artifacts=[];"
	         "function wantNames(w){return [{kind:'proof',name:'work-proof-'+w+'.html',label:'Proof'},{kind:'links',name:'work-links-'+w+'.txt',label:'Links'},{kind:'failure',name:'work-failure-'+w+'.html',label:'Failure'}];}"
	         "async function listOutbox(sess){try{const d=await jget(apiSess('/api/lab/outbox',sess)); return (d && d.outbox)?d.outbox:[];}catch(e){return []}}"
	         "async function refresh(){const st=document.getElementById('pf-st'); const status=document.getElementById('pf-status'); const ul=document.getElementById('pf-list'); const approveBtn=document.getElementById('pf-approve');"
	         "st.textContent='Loading…'; status.textContent='(loading…)'; ul.textContent='(loading…)';"
	         "document.getElementById('pf-required').style.display='none';"
	         "let canApprove=false;"
	         "try{const s=await jget(api('/api/work-prod/state?id='+encodeURIComponent(String(workId||'')))); const ss=String(s.status||'(unknown)'); status.textContent=ss; prUrl=String(s.pr_url||''); canApprove=(ss==='proof_ready');}catch(e){status.textContent='(no state)'; prUrl=''; canApprove=false;}"
	         "approveBtn.disabled=!canApprove;"
	         "if(prUrl){document.getElementById('pf-open-pr').href=prUrl; document.getElementById('pf-open-pr').style.display='inline-flex';}else{document.getElementById('pf-open-pr').style.display='none';}"
	         "const [stable,canary]=await Promise.all([listOutbox(STABLE), listOutbox(CANARY)]);"
	         "const wanted=wantNames(String(workId||'')); artifacts=[];"
	         "for(const x of wanted){const fs1=stable.find(f=>String(f.name||'')===x.name); if(fs1) artifacts.push(Object.assign({sess:STABLE},x,fs1));"
	         "const fs2=canary.find(f=>String(f.name||'')===x.name); if(fs2) artifacts.push(Object.assign({sess:CANARY},x,fs2));}"
	         "ul.innerHTML='';"
	         "if(!artifacts.length){ul.innerHTML='<div class=\"sub\">No proof artifacts in outbox yet.</div>'; st.textContent=''; return;}"
	         "artifacts.sort((a,b)=>(b.mtime_ms||0)-(a.mtime_ms||0));"
	         "for(const it of artifacts){const li=document.createElement('li'); li.className='item'; const left=document.createElement('div');"
	         "const nm=document.createElement('div'); nm.className='name'; nm.textContent=it.name;"
	         "const sb=document.createElement('div'); sb.className='sub'; sb.textContent='session '+it.sess+' · '+fmtKB(it.size_bytes);"
	         "left.appendChild(nm); left.appendChild(sb); const right=document.createElement('div'); right.className='row';"
	         "const bV=document.createElement('button'); bV.className='btn'; bV.type='button'; bV.textContent='View'; bV.addEventListener('click', async ()=>{const ref='outbox:'+it.sess+'/'+it.name; setRef(ref); await viewOutbox(it.sess,it.name,it.label||'Outbox');});"
	         "const bC=document.createElement('button'); bC.className='btn'; bC.type='button'; bC.textContent='Copy ref'; bC.addEventListener('click', async ()=>{const ref='outbox:'+it.sess+'/'+it.name; setRef(ref); await copyText(ref);});"
	         "right.appendChild(bV); right.appendChild(bC); li.appendChild(left); li.appendChild(right); li.addEventListener('click', ()=>{const ref='outbox:'+it.sess+'/'+it.name; setRef(ref);}); ul.appendChild(li);} st.textContent='Ready.';}"
	         "function pick(kind){const k=String(kind||''); const f=artifacts.filter(x=>x.kind===k); if(!f.length) return null; f.sort((a,b)=>(b.mtime_ms||0)-(a.mtime_ms||0)); return f[0];}"
	         "async function openKind(kind){const it=pick(kind); const st=document.getElementById('pf-st'); if(!it){st.textContent='Not found.'; return;} const ref='outbox:'+it.sess+'/'+it.name; setRef(ref); st.textContent='Opening…'; await viewOutbox(it.sess,it.name,it.label||'Outbox');}"
	         "async function approve(){const btn=document.getElementById('pf-approve'); const st=document.getElementById('pf-st'); btn.disabled=true; st.textContent='Approving…';"
	         "try{const resp=await fetch(api('/api/work-prod/approve-proof'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:String(workId||'')})});"
	         "const data=await resp.json().catch(()=>({}));"
	         "if(!resp.ok){st.textContent=(data && data.message)?data.message:('Failed: HTTP '+resp.status);"
	         "if(data && data.required_href){const a=document.getElementById('pf-required'); a.href=data.required_href; a.style.display='inline-flex';} btn.disabled=false; return;}"
	         "const url=String(data.pr_url||'').trim(); prUrl=url; if(url){setRef(url); await copyText(url); document.getElementById('pf-open-pr').href=url; document.getElementById('pf-open-pr').style.display='inline-flex'; st.textContent='PR ready.';}else{st.textContent='Done.';}"
	         "refresh().catch(()=>{});"
	         "}catch(e){st.textContent='Failed.'; btn.disabled=false;}}"
	         "document.getElementById('pf-set').addEventListener('click', ()=>{setWork(document.getElementById('pf-w').value||''); showReview().catch(()=>{});});"
	         "document.getElementById('pf-refresh').addEventListener('click', ()=>refresh().catch(()=>{}));"
	         "document.getElementById('pf-open-proof').addEventListener('click', ()=>openKind('proof').catch(()=>{}));"
	         "document.getElementById('pf-open-links').addEventListener('click', ()=>openKind('links').catch(()=>{}));"
	         "document.getElementById('pf-open-failure').addEventListener('click', ()=>openKind('failure').catch(()=>{}));"
	         "document.getElementById('pf-copy-pr').addEventListener('click', async ()=>{if(prUrl){setRef(prUrl); await copyText(prUrl);}});"
	         "document.getElementById('pf-approve').addEventListener('click', ()=>approve().catch(()=>{}));"
	         "refresh().catch(()=>{});"
	         "}"

	         "async function showSignoff(){"
	         "if(!workId){openPanel('Sign off', '<div class=\"sub\">Pick a work first (Work → Select).</div>'); return;}"
	         "openPanel('Sign off', '<div class=\"sub\">Creates a GitHub PR for <code>'+workId+'</code> after verifying the correct proctor channel.</div><div style=\"height:10px\"></div>' +"
	         "'<div class=\"row\"><div class=\"sub\">Required:</div><div class=\"sub\" id=\"sr\">(checking…)</div><div class=\"sub\" style=\"margin-left:8px\">Current:</div><div class=\"sub\" id=\"sc\">(checking…)</div></div>' +"
	         "'<div style=\"height:10px\"></div><div class=\"row\"><a class=\"btn\" id=\"so-link\" href=\"#\" target=\"_blank\" rel=\"noreferrer\" style=\"display:none\">Open required</a>' +"
	         "'<button class=\"btn primary\" id=\"so-go\" type=\"button\">Create PR</button><button class=\"btn\" id=\"so-copy\" type=\"button\">Copy PR URL</button><span class=\"sub\" id=\"so-st\"></span></div>' +"
	         "'<div class=\"sub\" style=\"margin-top:10px\">This runs lightweight repo checks and then creates (or finds) the PR.</div>');"
	         "let lastUrl='';"
	         "const st=document.getElementById('so-st');"
	         "const link=document.getElementById('so-link');"
	         "const btn=document.getElementById('so-go');"
	         "const copy=document.getElementById('so-copy');"
	         "copy.addEventListener('click', async ()=>{if(lastUrl){setRef(lastUrl); await copyText(lastUrl);}});"
	         "try{"
	         "const r=await jget(api('/api/work/requirements?id='+encodeURIComponent(workId)));"
	         "document.getElementById('sr').textContent=r.required_proctor||'?';"
	         "document.getElementById('sc').textContent=r.current_proctor||'?';"
	         "if(!r.ok){btn.disabled=true; st.textContent='Wrong proctor channel.'; if(r.required_href){link.href=r.required_href; link.style.display='inline-flex';}}"
	         "else{btn.disabled=false; st.textContent='Ready.'; if(r.required_href){link.href=r.required_href; link.style.display='inline-flex'; link.textContent='Open required';}}"
	         "}catch(e){st.textContent='Failed to check requirements.';}"
	         "btn.addEventListener('click', async ()=>{"
	         "btn.disabled=true; st.textContent='Creating PR…';"
	         "try{"
	         "const resp=await fetch(api('/api/work/signoff'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:workId})});"
	         "const data=await resp.json().catch(()=>({}));"
	         "if(!resp.ok){"
	         "st.textContent=(data && data.message)?data.message:('Failed: HTTP '+resp.status);"
	         "btn.disabled=false;"
	         "if(data && data.required_href){link.href=data.required_href; link.style.display='inline-flex';}"
	         "return;"
	         "}"
	         "lastUrl=String(data.pr_url||''); setRef(lastUrl);"
	         "st.textContent=lastUrl?('PR: '+lastUrl):'Done.';"
	         "if(lastUrl) { await copyText(lastUrl); }"
	         "}catch(err){st.textContent='Failed.'; btn.disabled=false;}"
	         "});"
	         "}"
         ...
         "try{const r=await fetch(url,{method:'POST',body:fd}); if(!r.ok) throw new Error('HTTP '+r.status); const data=await r.json(); setRef(data.ref); await copyText(data.ref); st.textContent='Saved: '+data.ref;}catch(err){st.textContent='Failed: '+err;} });}"
         ...
         "if(!resp.ok) throw new Error('HTTP '+resp.status); const data=await resp.json(); lastRef=data.ref; setWork(wid); setRef(lastRef); await copyText(lastRef); pst.textContent='Saved: '+data.ref;}catch(err){pst.textContent='Failed: '+err;}});"
         "document.getElementById('pb2').addEventListener('click', async ()=>{if(lastRef){setRef(lastRef); await copyText(lastRef);}});}"
         ...
         "document.getElementById('sa').addEventListener('click', ()=>{setActive(document.getElementById('sn').value); closePanel();});}"
	         ...
	         "document.getElementById('b-signoff').addEventListener('click', ()=>showSignoff().catch(()=>{}));"
         ...
         "document.getElementById('b-clear').addEventListener('click', async ()=>{try{await fetch(api('/api/lab/terminal/clear'),{method:'POST'});}catch(e){} });"
	         "document.getElementById('refbox').addEventListener('input', (e)=>{refText=String((e.target && e.target.value)||'');});"
	         "document.getElementById('b-ref-copy').addEventListener('click', async ()=>{const rb=document.getElementById('refbox'); const t=rb?rb.value:''; setRef(t); await copyText(t);});"
	         "document.getElementById('b-ref-paste').addEventListener('click', async ()=>{const rb=document.getElementById('refbox'); const t=rb?rb.value:''; await pasteToTerm(t);});"
