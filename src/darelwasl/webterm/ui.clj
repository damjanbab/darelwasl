...
(defn lab-page
  ...
  (let [{:keys [lab-stable-session lab-canary-session terminal-count lab-max-upload-bytes lab-default-history-lines tmux-prefix]} cfg
    ...
    (str "<!doctype html><html><head><meta charset=\"utf-8\" />"
         ...
         "<button class=\"btn\" id=\"b-review\" type=\"button\">Review</button>"
         ...
         "<span class=\"meta\" id=\"meta\">" ui-role " · build " build-stamp "</span>"
         "</div>"
         ...
         "function setWork(id){workId=String(id||'').trim(); setCookie('dw_work_id', workId); const el=document.getElementById('active-work'); if(el) el.textContent=workId?workId:'(none)'}"
         ...
         "if(!workId){openPanel('Review', '<div class=\"sub\">Pick a work first (Work → Select).</div>'); return;}"
         "openPanel('Review latest', '<div class=\"sub\">Opens the newest document under the selected work.</div><div style=\"height:10px\"></div><div class=\"row\"><button class=\"btn\" id=\"rv\" type=\"button\">Open newest</button><button class=\"btn\" id=\"rm\" type=\"button\">Mark reviewed</button><button class=\"btn\" id=\"rc\" type=\"button\">Copy ref</button><span class=\"sub\" id=\"rst\"></span></div>');"
         "let lastRef='';"
         "async function pickNewest(){const data=await jget(api('/api/library/list?work='+encodeURIComponent(workId))); const items=(data.items||[]);"
         "const pref=(it)=>{const e=extLower(it.name); if(e==='.pdf') return 0; if(isImg(e)) return 1; if(isText(e)) return 2; return 9;};"
         "items.sort((a,b)=>{const pa=pref(a), pb=pref(b); if(pa!==pb) return pa-pb; return (b.mtime_ms||0)-(a.mtime_ms||0)});"
         "const it=items[0]; if(!it) return null; return it.ref;}"
         "async function openNewest(){const st=document.getElementById('rst'); st.textContent='Loading…';"
         "try{const ref=await pickNewest(); if(!ref){st.textContent='No files for this work yet.'; return;} lastRef=ref; st.textContent='Opened: '+ref; await viewRef(ref,'Review');}catch(e){st.textContent='Failed: '+e;}}"
         "document.getElementById('rv').addEventListener('click', ()=>openNewest());"
         "document.getElementById('rc').addEventListener('click', async ()=>{if(lastRef) await copyText(lastRef);});"
         "document.getElementById('rm').addEventListener('click', async ()=>{const st=document.getElementById('rst');"
         "if(!lastRef){st.textContent='Open a file first.'; return;} st.textContent='Saving…';"
         "try{const ts=(new Date()).toISOString(); const name='review-'+ts.replace(/[:.]/g,'-')+'.md';"
         "const content='# Reviewed\\n\\n- work: '+workId+'\\n- ref: '+lastRef+'\\n- at: '+ts+'\\n';"
         "const resp=await fetch(api('/api/library/paste'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({work:workId,name:name,content:content})});"
         "if(!resp.ok) throw new Error('HTTP '+resp.status); st.textContent='Reviewed.';}catch(e){st.textContent='Failed: '+e;}});"
         "openNewest();"
         "}"
         ...
         "const es=document.getElementById('active-session'); if(es) es.textContent=String(nn); const et=document.getElementById('active-tmux'); if(et) et.textContent=tmuxName(nn);"
         ...
         "document.getElementById('b-review').addEventListener('click', ()=>showReview().catch(()=>{}));"
