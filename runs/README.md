...
Avoid adding tasks mid-run unless a blocker makes it impossible to proceed; prefer queuing follow-ups for the next run. Each run works on its own branch (`run/<run-id>`); task work happens on sub-branches (`run/<run-id>/<task-id>`) with PRs into the run branch. When a run is complete, merge the run branch to `main` via PR, archive the run file (e.g., move to `runs/archive/`), and start a new run file for subsequent work.
