...
## Generated Inventory
- `docs/system.generated.md` is auto-generated by `scripts/generate-docs.sh`. Do not edit manually.
- `docs/catalog.edn` is the machine-readable catalog for code + tooling references.
...
- Terminal config envs:
  ...
  - `TERMINAL_MAX_RUNNING_SESSIONS` (optional cap on concurrently running sessions)
  - `TERMINAL_POLL_MS` (output polling interval)
  - `TERMINAL_MAX_OUTPUT_BYTES` (per poll)
  - `TERMINAL_TX_LOG_MAX_BYTES` (optional max size before tx log rotation/compression)
...
- Datomic role isolation: set `DATOMIC_ROLE` (ex: `app`, `import`, `telegram`, `seed`) and optional `DATOMIC_INSTANCE` to suffix the DB name and storage dir (`data/datomic/<role[-instance]>`, `darelwasl-<role[-instance]>`). `:mem` storage stays in-memory. Wrapper scripts `scripts/run-service.sh`, `scripts/run-import.sh`, `scripts/run-seed.sh`, `scripts/run-tg-dev.sh` set role defaults.
