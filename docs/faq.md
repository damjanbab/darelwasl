...
- Hetzner deploy plan: app will run on Debian host `77.42.30.144` as user `darelwasl`, repo at `/opt/darelwasl`, env at `/etc/darelwasl/app.env`, Datomic storage `/var/lib/darelwasl/datomic`, binding `APP_PORT=3000`; add SSH key before prep. Inbound 22/3000 only (80/443 reserved for future proxy); outbound SMTP ports 25/465 are blocked by provider.
