...
Use this document to capture recurring gotchas, clarifications, and instructions that do not fit cleanly in `docs/system.md`. Keep entries concise and link to capability IDs, tasks, or files when relevant.
...
- Auth sessions: session cookie is backed by the on-disk store (`SESSION_STORE_PATH`); server restarts keep sessions.
...
- Telegram integration: keep `TELEGRAM_BOT_TOKEN`/`TELEGRAM_WEBHOOK_SECRET`/`TELEGRAM_WEBHOOK_BASE_URL` in env; webhook must include `X-Telegram-Bot-Api-Secret-Token` and is disabled by default (enable with `TELEGRAM_WEBHOOK_ENABLED`). No live Telegram calls in CI—stub HTTP and use sample webhook payloads. Chat binding uses single-use link tokens (`POST /api/telegram/link-token`) which expire (`TELEGRAM_LINK_TOKEN_TTL_MS`, default 15m) and stores `:user/telegram-chat-id`; `/stop` clears it.
